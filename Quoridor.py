...
def bellman_ford(start, end):
    return networkx.bellman_ford_path(Board_Graph, start, end)
    distancias = dict()
    anterior = dict()
...
def chooseMovement():
    if (last_movements[1] is None) and (last_movements[0] == last_movements[1]):
        curmove = 1 if (last_movements[1] == 0) else 1
    else:
...
def moveOnTheBoard(bot_pos):
    ...
    bot_pos = path[0][1]  # actualizamos la posicion del bot como atributo.
    return bot_pos
